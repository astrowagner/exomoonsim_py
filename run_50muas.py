...
Run from the repo root (the folder that contains the `exomoonsim/` package):
    python run_50muas.py

It uses ALL CPU cores and writes results + figure into runs/.
To change trials or precision, edit the four marked lines below.
"""
import os

# Keep each worker single-threaded so N processes don't oversubscribe the BLAS
# (each trial is dominated by numpy bincount + tiny linear solves).
...
print(f"  precision = {precision*1e6:.0f} muas   pend = {base.pend:.0f} d   "
      f"duration = {base.duration_days/365.25:.1f} yr   "
